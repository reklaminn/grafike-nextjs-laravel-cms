...
@section('title', $tag->exists ? 'Etiket Düzenle' : 'Yeni Etiket')

@section('content')
<div class="flex items-center gap-3 mb-6">
    <a href="{{ route('admin.tour-tags.index') }}" class="text-gray-400 hover:text-gray-600">
        <i class="fas fa-arrow-left"></i>
    </a>
    <h1 class="text-2xl font-bold text-gray-800">
        {{ $tag->exists ? 'Etiket Düzenle' : 'Yeni Pazarlama Etiketi' }}
    </h1>
</div>
...
<form method="POST"
      action="{{ $tag->exists ? route('admin.tour-tags.update', $tag) : route('admin.tour-tags.store') }}"
      class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-xl shadow-sm border p-5 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Sadece harf, rakam, tire.</p>
            </div>
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $tag->sort_order ?? 0) }}"
        ...
        <div class="flex items-center">
            ...
            <label for="is_active" class="ml-2 text-sm text-gray-700">Aktif (frontend'de görünür)</label>
        </div>
    </div>
        ...
        @foreach($languages as $i => $lang)
            ...
            <div class="border border-gray-200 rounded-lg p-4 space-y-3">
                ...
                <input type="text" name="translations[{{ $i }}][name]"
                       ...
                       placeholder="Etiket adı (örn. Erken Rezervasyon)"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                <textarea name="translations[{{ $i }}][description]" rows="2"
                          placeholder="Açıklama (frontend tooltip / filtre yardımı)"
