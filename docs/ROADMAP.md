...
| 3.5 ✅ | Redis prompt caching | **BİTTİ (2026-06-13)** — `AiPromptCache` servisi: tenant-scoped key `hash(tenant+provider+model+system+messages+temp+max_tokens)`, `Cache::lock` stampede koruması, görseller key'de base64 yerine sha256 ile. `AiModelRouter::dispatchAndRecord` cache ile sarmalandı (gerçek fallback gövdesi `dispatch()`'e taşındı). Per-feature `cache_ttl` (config/ai.php): seo.meta 1440dk, page.translate 10080dk, media.alt 1440dk; yaratıcı feature'lar (page.create/block.template/library.rewrite/block.edit) 0=kapalı. Cache hit → `AiQuotaService::recordCacheHit` ücretsiz satır (token/cost=0, kotaya yazmaz; `metadata.cache_hit`+`saved_tokens`+`saved_cost_usd`). Ayrıca Anthropic **native prompt caching**: `AnthropicProvider::buildSystem` uzun stabil system'e (≥12000 char) `cache_control: ephemeral` ekler (~%90 input indirimi; `cache_read_input_tokens` zaten harvest ediliyordu). Dashboard'a "Yanıt Önbelleği" widget'ı (`AiUsageReporter::cacheStats` — hit rate + tasarruf, DB-agnostik). 22 yeni test (AiPromptCacheTest 10, AiModelRouterTest +4, AiQuotaServiceTest +2, AiUsageReporterTest +2). Config: `AI_CACHE_ENABLED`, `AI_PROMPT_CACHE_ENABLED`, `AI_CACHE_LOCK_WAIT`. |
