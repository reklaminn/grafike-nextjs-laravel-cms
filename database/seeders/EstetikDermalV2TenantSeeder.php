 ...
 * NOT: v1 ile aynı slug'ları kullanır → çalıştırınca v1 sayfalarını TEMA 2 ile DEĞİŞTİRİR.
 * İki tasarımı yan yana karşılaştırmak için ayrı bir staging tenant'ta çalıştırın.
 * Önce: V2ThemeSeeder + V2ChromeSeeder (central) çalışmış olmalı.
 */
class EstetikDermalV2TenantSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $cb=$theme?SectionTemplate::where('theme_id',$theme->id)->where('type','content-block')->where('variation','v2-free-html')->first():null;
        ...
        foreach($pages as $p){
            Page::updateOrCreate(['slug'=>$p['slug'],'language_id'=>$langId],
                ['title'=>$p['title'],'status'=>'published','show_in_menu'=>$p['show_in_menu'],
                 ...
                 'sections_json'=>[['id'=>'cb_'.$p['slug'],'type'=>'content-block','variation'=>'v2-free-html',
                   'render_mode'=>'html','section_template_id'=>$cb->id,'is_active'=>true,'sort_order'=>1,
                   'content'=>['html'=>$p['html']]]]]
            );
        }
        $this->command?->info('Tema 2: '.count($pages).' sayfa kuruldu.');
    }
}
